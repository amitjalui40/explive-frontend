    ...
    <main className="w-full bg-zinc-50 dark:bg-zinc-950 overflow-hidden">
      ...
      <div className="w-full max-w-480 mx-auto px-6 md:px-12 xl:px-24">
        <div className="flex items-start justify-between pb-6 border-b border-zinc-200 dark:border-white/5">
          <div className="flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-3">
            <span className="text-xs font-bold tracking-[0.3em] uppercase text-amber-500">About</span>
      ...
      <section className="w-full max-w-480 mx-auto px-6 md:px-12 xl:px-24 pt-16 md:pt-24 pb-12 md:pb-20">
        <h1 className="text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold text-zinc-900 dark:text-white leading-tight tracking-tight">
          ...
          <span className="text-amber-500 dark:text-amber-400">live</span> experiences.
        </h1>
      </section>
        ...
        <div className="flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-y-3">
          {eventCategories.cards.map((card, i) => (
            <React.Fragment key={card.id}>
              <span className="text-2xl md:text-3xl lg:text-4xl font-black tracking-tighter text-zinc-900 dark:text-white hover:text-amber-500 dark:hover:text-amber-400 transition-colors duration-300 cursor-default">
                {card.title}
              </span>
              {i < eventCategories.cards.length - 1 && (
                <span className="hidden sm:inline text-amber-500 text-sm mx-4 md:mx-6 select-none">◆</span>
              )}
            </React.Fragment>
          ))}
        </div>
      ...
      <section className="w-full py-24 md:py-36 bg-zinc-950 dark:bg-zinc-900">
        <div className="w-full max-w-480 mx-auto px-6 md:px-12 xl:px-24">
          ...
          <Link
            href="/contact"
            className="inline-flex items-center gap-3 text-sm font-bold tracking-widest uppercase text-white border border-white/25 px-8 py-4 rounded-full hover:bg-amber-500 hover:border-amber-500 transition-all duration-300 group"
          >
            Get In Touch
